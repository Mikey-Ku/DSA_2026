...
/**
 * Runs both scheduler approaches and checks that each gives the expected answer.
 * If a conflict is returned, this also checks that it contains two input meetings
 * that really overlap.
 */
...
class ConflictDetectionTest {
    ...
    fun `sorting still finds a conflict when the overlapping pair is not adjacent`() {
        // The sorting approach should still find a conflict in this mixed schedule.
        val meetings = listOf(
...
class AlgorithmEquivalenceTest {
    /**
     * Builds many sample schedules and checks that both approaches agree.
     */
    @Test
    fun `both algorithms agree on random schedules`() {
        ...
        repeat(2000) { trial ->
            // Random start times and lengths create many different schedules.
